...
class D implements FixtureInterface
{
    public function load(ObjectManager $manager)
    {
        $category=$manager->getRepository('AppBundle:Category')->findAll();
        $persons=$manager->getRepository('AppBundle:Person')->findAll();
        for($i=0;$i<10;$i++){
            $p = new Post();
            $p->setTitle($this->titulos()."$i");
            ...
            $p->setCategory($cat);
            $cat->addPost($p);
            $p->setPerson($persons[array_rand($persons)]);
            $p->setViews(0);
            $p->setChecked(false);
            $manager->persist($p);
            $manager->persist($cat);
    ...
    public function lorem_short(){
        $stg='Lorem ipsum dolor sit amet, 
